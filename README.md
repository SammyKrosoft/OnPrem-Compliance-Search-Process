...
And then below is the process expanded and commented in PowerShell code with a sample query using the `sent>=` and `sent<=` properties to get results between two dates, and the `attachmentnames` property to filter on specific attachments:
